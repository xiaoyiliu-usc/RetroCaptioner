...
def canonical_smiles(smi, canonical=True, root=-1):
    mol = Chem.MolFromSmiles(smi)
    if mol is not None:
        return Chem.MolToSmiles(mol, isomericSmiles=True, rootedAtAtom=root, canonical=canonical)
    else:
...
def get_forward_rsmiles(data):
    ...
    for k in range(times):
        ...
        random.shuffle(tmp)
        reactant_k, reactant_roots_k, rea_atom_map_numbers_k = [i[0] for i in tmp], [i[1] for i in tmp], [i[2] for i in
                                                                                                          tmp]
        aligned_reactants = []
        aligned_products = []
...
def get_retro_rsmiles(data):
    ...
    for k in range(times):
        ...
        if cano_atom_map is None:
            ...
            return return_status
        pro_smi = clear_map_canonical_smiles(product, canonical=False, root=pro_root)
        pro_smi_w = canonical_smiles(product, canonical=False, root=pro_root)
        ...
        for i, rea_map_number in enumerate(rea_atom_map_numbers):
            for j, map_number in enumerate(cano_atom_map):
                ...
                if map_number in rea_map_number:
                    ...
                    rea_smi = clear_map_canonical_smiles(reactant[i], canonical=False, root=rea_root)
                    rea_smi_wclean = canonical_smiles(reactant[i], canonical=False, root=rea_root)
                    aligned_reactants.append(rea_smi)
                    aligned_reactants_w.append(rea_smi_wclean)
        ...
        sorted_reactants_w = sorted(list(zip(aligned_reactants_w, aligned_reactants_order)), key=lambda x: x[1])
        aligned_reactants_w = [item[0] for item in sorted_reactants_w]
        reactant_smi_w = ".".join(aligned_reactants_w)

        # product_tokens = smi_tokenizer(pro_smi)
        # reactant_tokens = smi_tokenizer(reactant_smi)

        # product_tokens_w = smi_tokenizer(pro_smi_wclean)
        # reactant_tokens_w = smi_tokenizer(reactant_smi_w)

        return_status['src_data'].append(pro_smi)
        return_status['tgt_data'].append(reactant_smi)
        return_status['src_data_w'].append(pro_smi_w)
        return_status['tgt_data_w'].append(reactant_smi_w)
    ...
    # edit_distances = []
    # for src, tgt in zip(return_status['src_data'], return_status['tgt_data']):
    #     edit_distances.append(textdistance.levenshtein.distance(smi_tokenizer(src).split(), smi_tokenizer(tgt).split()))
    # return_status['edit_distance'] = np.mean(edit_distances)
    return return_status


if __name__ == '__main__':
    data_path = './data/data/uspto50k/'
    data_name = 'train.csv'
    df = pd.read_csv(os.path.join(data_path, data_name))
    results = []

    for index, row in df.iterrows():
        print(f'processing {index} / {len(df)} molecules')
        reactant, reagent, product = row['rxn_smiles'].split(">")
        pt = re.compile(r':(\d+)]')
        rids = sorted(re.findall(pt, reactant))
        pids = sorted(re.findall(pt, product))
        ...
        src_smi = clear_map_canonical_smiles(input)
        tgt_smi = clear_map_canonical_smiles(output)
        ...
        if not mapping_check:
            print(
                "The quality of the atom mapping may not be good enough, which can affect the effect of root alignment.")
        ...
        for index, (src, tgt) in enumerate(zip(res['src_data'], res['tgt_data'])):
            ...
            reactant_smiles = ''.join(tgt.split())
            product_smiles = ''.join(src.split())
            rxn_smiles = f"{reactant_smiles}>>{product_smiles}"

            # print(f"R-SMILES input:{product_smiles}")
            # print(f"R-SMILES output:{reactant_smiles}")
            # print(f"R-SMILES rxn:{rxn_smiles}")
    ...
    result_df.to_csv(os.path.join(data_path, 'val_R.csv'), index=False)
    print(f"Processed reactions are saved to {data_path}")
